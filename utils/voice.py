...
def get_whisper_model():
    """
    Singleton factory: Only loads the model the first time it is needed.
    """
    global _model_instance
    if _model_instance is None:
        print("--- 🧠 Loading Whisper AI Model (Lazy Load)... ---")
        # Set device to 'cuda' for RTX 5070 Ti, fallback to 'cpu'
        device = "cuda" if os.getenv("USE_CUDA", "true").lower() == "true" else "cpu"
        compute_type = "float16" if device == "cuda" else "int8"
        
        try:
            _model_instance = WhisperModel("small", device=device, compute_type=compute_type)
            print(f"--- Whisper Loaded on {device} ---")
        except Exception as e:
            print(f"Failed to load CUDA, falling back to CPU: {e}")
...
def record_until_silence(fs=16000, silence_threshold=0.01, silence_duration=1.5):
    ...
    # Increase blocksize/latency slightly to prevent buffer underflows on Linux
